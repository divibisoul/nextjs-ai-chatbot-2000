...
test('N06 exposes exactly six peers and twelve directional channels', () => {
  assert.deepEqual(N06_PEERS, ['N01', 'N02', 'N03', 'N04', 'N05', 'N07']);
  assert.equal(N06_IN_CHANNELS.length, 6);
  assert.equal(N06_OUT_CHANNELS.length, 6);
  assert.equal(N06_CHANNELS.length, 12);
  assert.equal(new Set(N06_CHANNELS).size, 12);
});

test('N06 channel matrix is bidirectional for every peer', () => {
  assert.equal(N06_CHANNEL_MATRIX.length, 6);
