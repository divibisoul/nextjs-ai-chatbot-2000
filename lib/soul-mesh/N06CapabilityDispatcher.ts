...
import { supportsNucleus06Capability, type Nucleus06Capability } from '@/lib/soul-core/Nucleus06Capabilities';
...
export async function executeN06Capability(
  capability: string,
  payload: unknown,
  ...
  if (capability.startsWith('tool:')) {
    ...
    if (!NUCLEUS_05_TOOL_IDS.includes(toolId as (typeof NUCLEUS_05_TOOL_IDS)[number])) {
      throw new Error(`N06_UNKNOWN_TOOL:${toolId}`);
    }
    const tools = createNucleus05Tools(requireToolContext(context));
    const tool = tools[toolId as keyof typeof tools] as {
  ...
  if (supportsNucleus06Capability(capability)) {
    return nucleus06Processor.execute({ capability: capability as Nucleus06Capability, input: payload }, context);
  }
