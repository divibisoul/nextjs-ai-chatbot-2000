...
/** Mesh -> N06 runtime bridge. Reuses native AI/tool implementations. */
export async function executeN06Capability(capability: string, payload: unknown, context?: N06MeshExecutionContext) {
  ...
  if (capability === 'tool-execution') {
    if (!context?.session || !context?.dataStream) {
      throw new Error('N06_TOOL_CONTEXT_REQUIRED');
    }
    const input = payload as { toolId?: string; args?: unknown };
    if (!input?.toolId || !NUCLEUS_05_TOOL_IDS.includes(input.toolId as any)) {
      throw new Error(`UNKNOWN_TOOL:${input?.toolId ?? ''}`);
    }
