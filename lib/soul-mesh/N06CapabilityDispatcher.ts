...
export function getN06Capabilities(): readonly string[] {
  return [...NUCLEUS_06_TOOL_IDS.map((id) => `tool:${id}`), 'ai-pilot', 'artifact-processing', 'document-processing', 'context-orchestration', 'streaming', 'mesh-communication'];
}
...
export async function executeN06Capability(capability: string, payload: unknown, context?: N06MeshExecutionContext) {
  if (capability === 'ai-pilot') return nucleus06Processor.execute({ capability: 'ai-pilot', input: payload }, context);
  ...
  if (nucleus06Processor.supports(capability)) return nucleus06Processor.execute({ capability: capability as any, input: payload }, context);
