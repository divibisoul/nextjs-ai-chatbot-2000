...
export class N06PeerMeshBridge {
  ...
  constructor(options:{retries?:number; peers?:Partial<Record<Exclude<NucleusId,'N06'>,string>>} = {}) {
    ...
    for (const peer of PEERS) { const url = options.peers?.[peer] ?? process.env[envKey(peer)]; if (url) this.peers.set(peer,{url:url.replace(/\/+$/,''),healthy:false,failures:0,latencyMs:null,openedUntil:0}); }
  }
  ...
  async request(peer:Exclude<NucleusId,'N06'>,capability:string,payload:unknown,correlationId=randomUUID(),traceId=randomUUID()){
    if (!this.peers.has(peer)) throw new Error(`PEER_NOT_CONFIGURED:${peer}`);
    const started=Date.now(); let last:unknown;
    for(let attempt=0;attempt<=this.retries;attempt++){
      try{
        const result=await sendFromN06(peer,capability,payload,Number(process.env.SOUL_MESH_TIMEOUT_MS??30000));
        const state=this.peers.get(peer)!;state.healthy=true;state.failures=0;state.latencyMs=Date.now()-started;state.openedUntil=0;
        return {peer,payload:result,status:200,latencyMs:state.latencyMs,attempt,correlationId,traceId};
      }catch(error){last=error;const state=this.peers.get(peer)!;state.failures+=1;state.healthy=false;if(state.failures>=5)state.openedUntil=Date.now()+60000;if(attempt<this.retries)await new Promise(r=>setTimeout(r,150*(2**attempt)));}
    }
