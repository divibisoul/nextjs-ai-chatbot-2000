...
export function isSoulMeshMessage(value: unknown): value is SoulMeshMessage {
  ...
  return (
    ...
    (SOUL_NUCLEI as readonly string[]).includes(m.source as string) &&
    (SOUL_NUCLEI as readonly string[]).includes(m.target as string) &&
    m.source !== m.target &&
    ['request','response','event','error'].includes(m.kind as string) &&
    typeof m.timestamp === 'number' && Number.isFinite(m.timestamp) &&
    (m.kind === 'event' || typeof m.capability === 'string') &&
    'payload' in m
  );
}
