...
const base = { contractVersion:'1.1.0', messageId:crypto.randomUUID(), source:'N06', target:'N01', timestamp:Date.now(), nonce:crypto.randomUUID(), correlationId:id, type:'CAPABILITY_REQUEST', payload:{capability:'mesh.register', payload:{nucleus:'N06',endpoint:n06||null,protocol:'soul-mesh/1',capabilities:['mesh.ping','mesh.health','mesh.discovery','mesh.delegate','mesh.combo']}} };
...
try {
  ...
  console.log(JSON.stringify({ok:true,source:message.source,target:message.target,contractVersion:message.contractVersion,correlationId:id,response:body},null,2));
