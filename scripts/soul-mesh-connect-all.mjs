...
function envelope(target, capability, payload = {}) {
  ...
  const base = { contractVersion:'1.1.0', messageId:crypto.randomUUID(), source:N06, target, timestamp:Date.now(), nonce:crypto.randomUUID(), correlationId, type:'CAPABILITY_REQUEST', payload:{ capability, payload } };
...
async function post(peer, message) {
  ...
  try {
    ...
    return { peer, configured:true, ok:response.ok, status:response.status, contractVersion:message.contractVersion, correlationId:message.correlationId, body };
  } catch (error) {
    return { peer, configured:true, ok:false, contractVersion:message.contractVersion, correlationId:message.correlationId, error:error instanceof Error ? error.message : String(error) };
  } finally { clearTimeout(timer); }
}

const results = await Promise.all(peers.map((peer) => post(peer, envelope(peer, 'mesh.ping', { requestedBy:N06 }))));
console.log(JSON.stringify({ source:N06, contractVersion:'1.1.0', testedAt:new Date().toISOString(), results, configured:results.filter(r=>r.configured).length, reachable:results.filter(r=>r.ok).length }, null, 2));
