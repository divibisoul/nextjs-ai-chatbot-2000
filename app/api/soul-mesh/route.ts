...
export async function POST(request: Request) {
  ...
  if (!message || message.protocol !== 'soul-mesh/1' || !message.id || !message.correlationId || !NUCLEI.has(message.source) || !NUCLEI.has(message.target) || message.target !== NUCLEUS_ID || message.source === NUCLEUS_ID || !message.capability) return NextResponse.json({ error: 'INVALID_SOUL_MESH_MESSAGE' }, { status: 400 });
