...
function authorized(request: Request): boolean { const token = process.env.SOUL_MESH_TOKEN; return !token || request.headers.get('authorization') === `Bearer ${token}`; }
...
export async function POST(request: Request) {
  ...
  if (message.capability === 'mesh.ping') return NextResponse.json({ protocol: 'soul-mesh/1', id: crypto.randomUUID(), correlationId: message.correlationId, source: NUCLEUS_ID, target: message.source, kind: 'response', capability: 'mesh.ping', payload: { ok: true, handler: 'N06.mesh.ping', echoed: message.payload, processedAt: Date.now() }, timestamp: Date.now() } satisfies SoulMeshMessage);
